...
class eastclass(object):
    ...
    def get_images(self):
        ...
        for parent, dirnames, filenames in os.walk(self.test_data_dir):
    ...
    def cut_roi(self, image,box,im_fn,num):
        #w_lenght = np.linalg.norm(box[0] - box[1])
        #h_lenght = np.linalg.norm(box[3] - box[0])
        im_file_path = os.path.join(
            self.output_dir,
            '{}_img_{}_box.jpg'.format(
                os.path.basename(im_fn).split('.')[0],num))
        #print(image.shape)
                    ...
                    if boxes is not None:
                        ...
                        with open(res_file, 'w') as f:
                            for i, box in enumerate(boxes):
                                ...
                                im_file_path=self.cut_roi(im[:, :, ::-1], box, im_fn, i)
                                # print(im_fn)
                                # print(im_file_path)

                                text = recongnize_chinese_pdf.recognize(image_path=im_file_path,weights_path='model/checkpoint/crnnch/',char_dict_path='crnn/data/char_dict/char_dict_cn.json',ord_map_dict_path='crnn/data/char_dict/ord_map_cn.json',output_path='data/outtxt/')
                                print(text)
                                x= int(box[1, 1])
                                y= int(box[0, 1])
                                #text = "{}:{:.3f}".format(label, number)
                                img = self.cv2ImgAddText(im[:, :, ::-1], text, x-10, y - 5, (255,0,0), 10)
                                #cv.putText(im[:, :, ::-1], text, (x, y - 5), cv.FONT_HERSHEY_COMPLEX, fontScale=0.5,color=(0,255,0), thickness=1)
